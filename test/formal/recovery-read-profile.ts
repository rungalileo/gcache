import { encodeFrame } from "../fake-redis.js";
import type { Input } from "./behavior-driver.js";
import type { Profile } from "./feature-profile.js";

const seedAges = [0, 999, 1000, 4999, 5000, 3_600_001, 7_200_000, 0, 1000, 1000, 0, 0, 4999, 0, 0, 1000, 0, 0, 1000] as const;
...
function seedInput(choice: number): Input {
  const base = { op: "seed" as const, ageMs: seedAges[choice]!, ttlMs: choice === 11 || choice === 15 ? 1000 : 60_000 };
  if (choice === 18) return base;
  if (choice === 8 || choice === 12 || choice === 13) return { ...base, payloadHex: compressedOne };
  if (choice === 9 || choice === 14) return { ...base, payloadHex: corruptCompressed };
  if (choice === 16) return { ...base, frameHex: "0100200000000000010031" };
  if (choice === 17) {
    const frame = encodeFrame("1", Date.now());
    frame[0] = 2;
    return { ...base, frameHex: frame.toString("hex") };
  }
  if (choice === 10) return { ...base, frameHex: encodeFrame("1", Date.now(), 255).toString("hex") };
