...
| C23 | Read deadline precedence: runtime → operation → instance → 50 ms; source defaults to 60 seconds and starts only at source invocation | S: library/instance/operation/runtime `read deadline precedence`, `fallback deadline starts after remote read completes`, `library source deadline defaults to sixty seconds`; coalescing model; G: effects (library/instance/operation/runtime read budget precedence) |
| C24 | Late followers inherit remaining read/source time and outcome; uncoalesced work owns independent budgets | S: `late follower inherits remaining read deadline`, `late follower inherits remaining fallback deadline`; coalescing model; G: independent (per-call read/source budgets and late completion) |
