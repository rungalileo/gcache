import { cohortRamp } from "../cohort-inputs.mjs";
...
const sourceValues = [1, 2, undefined, "undefined", null, false, 0, ""];
export const runtimeBoundariesProfile = {
  explicitInputs: true,
  ...
  actions: {
    ...
    policy: {
      ...
      input: choice => {
        if (choice === 21)
          return { op: "policy", value: { requestLocal: false, ramp: { local: 0, remote: 0 }, staleOnErrorMaxAgeSec: 0, shadow: { ramp: 0 } } };
        if (choice === 12)
          return { op: "policy", value: null };
        if (choice === 13)
          return { op: "policy", value: { requestLocal: false } };
        if (choice === 18 || choice === 19)
          return { op: "policy", value: { ttlSec: choice === 18 ? { local: 60 } : { remote: 60 } } };
        if (choice === 20)
          return { op: "policy", value: { ttlSec: { local: 60 }, ramp: { local: 100 } } };
        if (choice >= 14) {
          const value = choice >= 16 ? null : "invalid";
          return { op: "policy", value: choice % 2 === 0
              ? { coalesce: value } : { requestLocal: value } };
        }
